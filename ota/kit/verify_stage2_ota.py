...
def radiotap_mpdu(packet: bytes) -> tuple[bytes | None, str]:
    ...
    if flags & RADIOTAP_F_FCS:
        ...
        if captured_fcs != expected_fcs:
            # FCS bytes present but not the CRC of the captured body: the
            # frame itself is still real evidence (e.g. a caller-supplied
            # marker FCS preserved over the air), so keep it matchable and
            # carry the fact as metadata instead of dropping the packet.
            # Listeners that flag the frame BADFCS (never validated) stay
            # excluded below.
            return body, "invalid"
        return body, "valid"
    return mpdu, "absent"
    ...
    for row, item in enumerate(expected):
        ...
        for column, packet in enumerate(candidates, 1):
            ...
            if normalize(packet.mpdu) == key:
                ...
                # The window must outrank byte-exactness: an out-of-window
                # retransmission copy (byte-exact) must never steal an A/B
                # phase binding from the in-window copy (which the FW may
                # have rewritten in Duration/TSF). v9 raised the in-window
                # bonus from +100 to +100_000 so it dominates the +1_000
                # exact bonus instead of being a soft preference.
                score = previous[column - 1] + 1_000_000 + \
                    100_000 * int(in_window) + 1_000 * int(exact)
                if score > best:
                    best = score
                    choice = 3
            current[column] = best
...
def selftest(manifest_path: pathlib.Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="stage2-ota-selftest-") as temporary:
        ...
        for index, case in enumerate(selected):
            ...
            if index == 1:
                # A byte-exact RETRANSMISSION copy just outside the window
                # (50us after window end, before the next case): the matcher
                # must still prefer the in-window rewritten copy (v9
                # window-over-exactness preference). Placement matters -
                # both candidates keep the alignment legal, so only the
                # weights decide.
                exact_copy = bytearray.fromhex(case["mpdu_hex"])
                capture_packets.append((base + 1_950_000, bytes(exact_copy)))
        ...
        report = verify(manifest_path, results_path, capture_path, matrix_path)
        if report["ota_observed"] != len(selected):
            raise AssertionError("dynamic ordered matcher lost rewritten/duplicate frames")
        matrix = [line.split("\t") for line in
                  matrix_path.read_text().splitlines()[1:]]
        exact_by_case = {row[0]: row[3] for row in matrix}
        if exact_by_case.get(selected[1]["case_id"]) != "0":
            raise AssertionError("out-of-window exact copy stole the phase binding")
    print(f"stage2_ota_selftest=PASS cases={len(selected)} duplicate=1 "
          f"duration_rewrite={len(selected)} window_over_exact=1")
